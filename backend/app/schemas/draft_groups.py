...
class DraftGroupLineSchema(Schema):
    ...
    quantity = fields.Float(required=True, validate=validate.Range(min=0.01, max=9999.99))
    uom = fields.String(
...
class DraftGroupCreateSchema(Schema):
    ...
    name = fields.String(allow_none=True, validate=validate.Length(max=200)) # Legacy alias
...
class DraftGroupUpdateSchema(Schema):
    """Schema for updating draft group (name and/or description)."""
