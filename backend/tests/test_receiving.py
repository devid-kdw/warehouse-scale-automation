...
from app.models import Stock, Batch, Transaction, User
...
class TestReceiveStockValidation:
    ...
    def test_receive_location_not_1(self, app, location, article, user):
        ...
        If location doesn't exist: LOCATION_NOT_FOUND
        If location exists but != 1: LOCATION_NOT_ALLOWED
        """
        with app.app_context():
            u = db.session.get(User, user)
            u.role = 'ADMIN'
            ...
            with pytest.raises(AppError) as exc:
                receive_stock(
                    ...
                    expiry_date=date.today() + timedelta(days=100),
                    actor_user_id=user,
                    location_id=2  # Not allowed in v1
                )
            
            # Location 2 doesn't exist in test DB, so we get NOT_FOUND
            # If it existed, we'd get NOT_ALLOWED
