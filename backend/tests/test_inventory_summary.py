...
def test_inventory_summary_filters(client, app, user, location, article):
    ...
    with app.app_context():
        art2 = Article(article_no='TEST-002', description='Test Article 2')
        app.extensions['sqlalchemy'].db.session.add(art2)
        app.extensions['sqlalchemy'].db.session.commit()
        
        batch2 = Batch(article_id=art2.id, batch_code='5678')
        app.extensions['sqlalchemy'].db.session.add(batch2)
        app.extensions['sqlalchemy'].db.session.commit()
