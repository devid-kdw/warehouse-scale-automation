...
export function AliasesEditor({ article, opened, onClose }: AliasesEditorProps) {
    ...
    const createMutation = useMutation({
        ...
        onSuccess: () => {
            notifications.show({ title: 'Uspjeh', message: 'Alias dodan', color: 'green', icon: <IconCheck size={16} /> });
            queryClient.invalidateQueries({ queryKey: ['aliases', article?.id] });
            form.reset();
        },
        onError: (err) => {
            notifications.show({ title: 'Greška', message: extractErrorMessage(err), color: 'red' });
        }
    });
    ...
    const deleteMutation = useMutation({
        ...
        onSuccess: () => {
            notifications.show({ title: 'Uspjeh', message: 'Alias uklonjen', color: 'green', icon: <IconCheck size={16} /> });
            queryClient.invalidateQueries({ queryKey: ['aliases', article?.id] });
        },
        onError: (err) => {
            notifications.show({ title: 'Greška', message: extractErrorMessage(err), color: 'red' });
    ...
    const form = useForm({
        initialValues: { alias: '' },
        validate: {
            alias: (val) => val.trim().length < 2 ? 'Alias prekratak' : null,
        },
    });
    ...
    return (
        <Modal opened={opened} onClose={onClose} title={`Aliasi: ${article?.article_no}`} centered>
            <LoadingOverlay visible={isLoading} />

            <Text size="sm" c="dimmed" mb="md">
                Upravljajte alternativnim nazivima/kodovima za ovaj artikl. Maks. 5 aliasa.
            </Text>
            ...
            <Table mb="md" withTableBorder>
                ...
                <Table.Tbody>
                    {aliases.length === 0 ? (
                        <Table.Tr>
                            <Table.Td colSpan={2} ta="center" c="dimmed">Nema aliasa</Table.Td>
                        </Table.Tr>
                    ) : (
                        aliases.map(a => (
                            <Table.Tr key={a.id}>
            ...
            {/* Add Form */}
            {isLimitReached ? (
                <Alert color="orange" title="Limit dostignut">
                    Maksimalno 5 aliasa po artiklu.
                </Alert>
            ) : (
                <form onSubmit={form.onSubmit(handleSubmit)}>
                    <Group align="flex-start" gap="xs">
                        <TextInput
                            placeholder="Unesite novi alias"
                            style={{ flex: 1 }}
            ...
            <Group justify="flex-end" mt="xl">
                <Button variant="default" onClick={onClose}>Zatvori</Button>
            </Group>
        </Modal>
    );
}
